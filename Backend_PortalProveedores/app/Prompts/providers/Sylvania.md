...
- **Series & Number (serie, numero)**: 
    - **Serie**: Find the label **"Serie:"** (usually upper right). Extract the COMPLETE alphanumeric code immediately following it. The serie is always **8 characters long** (e.g., `"C34DA1EA"`, `"A5766FF5"`). **CRITICAL: Do NOT truncate the serie**. If you see `"Serie: C34DA1EA"`, extract all 8 characters `"C34DA1EA"`, not just `"C34DA"`.
    - The serie is identical to the **first segment of the UUID** (the 8 characters before the first `-`). If you are unsure, cross-reference with the UUID.
