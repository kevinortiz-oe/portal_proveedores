...
class InvoiceController extends BaseController
{
    ...
    public function analyze()
    {
        ...
        foreach ($invoicesFound as $invoice) {
            ...
            // Clean no_pedido (remove VOL-, CEL-, PO prefixes) for the numeric field
            $cleanNoPedido = $invoice['no_pedido'] ?? '';
            $cleanNoPedido = preg_replace('/^(VOL-|CEL-|PO\s*)/i', '', (string)$cleanNoPedido);
            ...
            $invoice['no_pedido'] = $invoice['no_pedido'] ?? '';
            ...
            if (!empty($invoice['items'])) {
                foreach ($invoice['items'] as &$item) {
                    ...
                    if (!empty($item['oc_detalle'])) {
                        $cleanOc = preg_replace('/^(VOL-|CEL-|PO\s*)/i', '', (string)$item['oc_detalle']);
    ...
    public function save()
    {
        ...
        try {
            ...
            // Clean no_pedido (remove VOL-, CEL-, PO prefixes)
            $cleanNoPedido = $invoiceData->no_pedido ?? '';
            $cleanNoPedido = preg_replace('/^(VOL-|CEL-|PO\s*)/i', '', $cleanNoPedido);
            ...
            if (!empty($invoiceData->items)) {
                foreach ($invoiceData->items as $item) {
                    $itemInsert = $itemModel->insert([
                        ...
                        'oc_detalle' => !empty($item->oc_detalle) ? preg_replace('/^(VOL-|CEL-|PO\s*)/i', '', $item->oc_detalle) : null,
